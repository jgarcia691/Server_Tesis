...
export const postencargadocontroller = async (req, res) => {
  try {
    const { ci, nombre, apellido, telefono, password, email, id_sede } = req.body;

    if (!ci || !nombre || !apellido || !telefono || !password || !email || !id_sede) {
      return res.status(400).json({
        message: 'Todos los campos son obligatorios: ci, nombre, apellido, telefono, password, email, id_sede',
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        message: 'ci, telefono y sede deben ser números; nombre, apellido, email y password deben ser cadenas.',
      });
    }
...
export const putencargadocontroller = async (req, res) => {
  try {
    ...
    if (!ci || !nombre || !apellido || !email || !telefonoNumber || !password || !id_sede) {
      return res.status(400).json({
